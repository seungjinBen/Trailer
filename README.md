...
세 페이지 본문은 앱과 동일한 확정 원문(앱 소스의 `TERMS` · `LOCATION_TERMS` · `PRIVACY_POLICY`)을 옮긴 것입니다.
약관을 개정할 때는 **앱과 웹의 문구가 어긋나지 않도록** 양쪽을 함께 수정하고,
`doc-hero__meta` 의 시행일 · 최종 개정일도 같이 고치세요.

> 원문 md 를 모아둔 `legal-export/` 디렉터리는 저장소에서 제거했습니다(정적 호스팅 시 원문이 그대로 노출되기 때문).
> 개정 이력이 필요하면 웹사이트가 아닌 앱 저장소 쪽에서 관리하는 것을 권장합니다.
